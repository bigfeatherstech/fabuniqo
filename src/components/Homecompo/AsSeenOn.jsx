...
const AsSeenOn = () => {
  // Mock data for UGC (User Generated Content)
  const posts = [
    {
      id: 1,
      handle: "@elena_styles",
      ...
      size: "large" // control the grid span
    },
    {
      id: 2,
      handle: "@marc.vogue",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-16 text-center md:text-left gap-6">
          <div className="max-w-xl">
            <h2 className="text-4xl md:text-6xl font-light tracking-tighter text-zinc-900 leading-none">
              As Seen <span className="italic font-serif text-[rgb(209,167,67)]">On You</span>
            </h2>
        ...
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
          {posts.map((post) => (
            <div 
              key={post.id} 
              className="relative group overflow-hidden bg-zinc-100 rounded-sm cursor-crosshair break-inside-avoid"
            >
              {/* Image with subtle zoom on hover */}
              <img 
                src={post.image} 
                alt={post.handle}
                className="w-full object-cover transition-transform duration-1000 group-hover:scale-110 img-fluid"
              />
